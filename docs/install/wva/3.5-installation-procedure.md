...
After this we should see confirmation that our three resources were created:
...
spec:
  ...
  template:
    containers:
      - name: main
        image: quay.io/aipcc/rhaiis/cuda-ubi9:3.5.0-ea.2-1782155603 # If using Accelerators, use the corresponding inference-image
        resources:
          limits:
            cpu: '4'
            memory: 32Gi
...
It should be noted for your scaledObject should be showing as `READY=true`. Once the WVA controller starts reconciling the `scaledObject` it should also become `ACTIVE=True`. If you are not seeing that, your first debugging steps should be to `describe` these resources and check their `Status` sections of their yaml.
...
### Scaling Limiter
Documentation PR waiting to be merged https://github.com/llm-d/llm-d/pull/2010
  
### Observability - WVA Prometheus Metrics
There are a number of Prometheus metrics added to WVA for observability. You can review the metrics [here](https://github.com/red-hat-data-services/workload-variant-autoscaler/blob/rhoai-3.5-ea.2/docs/developer-guide/prometheus.md)
...
- If the import is succcessful, you should see a new dashboard called `WVA Operational Dashboard`. Click on the dashboard:
  ...
  - Under `namespace_label`, leave it as `exported_namespace`. See https://github.com/red-hat-data-services/workload-variant-autoscaler/blob/rhoai-3.5-ea.2/docs/user-guide/monitoring.md#understanding-namespace-labels-in-metrics for more details.
  
### Observability - WVA Prometheus Alert Rules
You can install WVA Prometheus alert rules as follows:
```bash
curl -sL https://raw.githubusercontent.com/opendatahub-io/workload-variant-autoscaler/refs/heads/main/config/components/prometheus-alerts/prometheusrule.yaml | oc apply -n OpenShift-monitoring -f -
```

Verify WVA Prometheus alert rules created:
```console
oc get prometheusrules controller-manager-alerts -n OpenShift-monitoring

NAME                        AGE
controller-manager-alerts   65m
```

Verify WVA Prometheus alert rules:
```bash
TOKEN=$(oc whoami -t)
THANOS=$(oc get route thanos-querier -n OpenShift-monitoring -o jsonpath='{.spec.host}')

curl -sk -G -H "Authorization: Bearer $TOKEN" "https://$THANOS/api/v1/rules" | jq '.data.groups[] | select(.name ==
  "wva.rules")
```

You should see similar to the following shortened output:
```console
{
  "name": "wva.rules",
  "file": "/etc/prometheus/rules/prometheus-k8s-rulefiles-0/openshift-monitoring-controller-manager-alerts-8478fa63-d99b-4a6a-ab73-075984af1f8a.yaml",
  "rules": [
    {
      "state": "inactive",
      "name": "WVAGPUResourceExhausted",
      "query": "wva_available_gpus == 0",
      "duration": 300,
      "labels": {
      ...
      "alerts": [],
      "health": "ok",
      "evaluationTime": 0.00003081,
      "lastEvaluation": "2026-07-22T16:08:49.553205994Z",
      "keepFiringFor": 0,
      "type": "alerting"
    },
  ...
...
### Upgrade from RHOAI 3.4
Before upgrading to 3.5, there are 2 WVA configmaps in `redhat-ods-applications`:
```bash
workload-variant-autoscaler-saturation-scaling-config
workload-variant-autoscaler-wva-variantautoscaling-config
```
After upgrading to 3.5, these 2 configmaps are renamed respectively to:
```bash
workload-variant-autoscaler-saturation-scaling-config
workload-variant-autoscaler-manager-config
```
If you made changes to the former 2 configmaps then you will also need to manually make the same changes to the latter 2 configmaps.

### Token-based Capacity Analyzer
In addition to the percentage-based saturation analyzer which is the default analyzer for WVA, there's a new token-based saturation analyzer in which demand and supply are in KV cache tokens.
#### Enable Token-based Capacity Analyzer
To enable, edit WVA saturation configmap:
```bash
kubectl edit cm -n redhat-ods-applications workload-variant-autoscaler-saturation-scaling-config
```
and append the `analyzers` section as follows:
```console
data:
  default: |
    ...
    ...
    - name: saturation
```
There is no need to restart WVA controller manager pod. To verify that the updated configmap is in effect, search the WVA manager log for `V2`:
```console
kubectl logs -n redhat-ods-applications -l app.kubernetes.io/name=workload-variant-autoscaler | fgrep V2

026-07-06T14:00:55Z    INFO    saturation/engine.go:816        Processing model (V2)   {"modelID": "unsloth/Meta-Llama-3.1-8B", "namespace": "autoscaling-example", "variantCount": 1, "groupKey": "unsloth/Meta-Llama-3.1-8B|autoscaling-example"}
2026-07-06T14:00:55Z    INFO    saturation/engine_v2.go:67      V2 saturation analysis completed   {"modelID": "unsloth/Meta-Llama-3.1-8B", "totalSupply": 13107, "totalDemand": 0, "utilization": 0, "requiredCapacity": 0, "spareCapacity": 0}
```

#### Token-based Capacity Analyzer Details
The details can be found [v2-analyzer.md](./v2-analyzer.md)